...
cnn = tf.keras.models.load_model('./cnn_for_standard_dogs.h5') # P352의 PG 8 -7의 결과
dog_species = pickle.load(open('dog_species_names.text','rb')) # P352의 PG 8 -7의 결과

class DogSpeciesRecognition(QMainWindow):
    
    def __init__(self):

        super().__init__() # QMainWindow Object에 대한 생성자 함수
        ...
        recognition = QPushButton('품종 인식',self)
        quitButton = QPushButton('나가기',self)


        fileButton.setGeometry(10,10,100,30)
        recognition.setGeometry(110,10,100,30)
        quitButton.setGeometry(510,10,100,30)


        fileButton.clicked.connect(self.pictureOpenFunction)
        recognition.clicked.connect(self.recognitionFunction)
        quitButton.clicked.connect(self.quitFunction)


    def pictureOpenFunction(self):

        filename = QFileDialog.getOpenFileName(self,'강아지 사진 읽기','./')
    ...
    def recognitionFunction(self):
        ...
        top5 = np.argsort(-res)[:5] # 상위 확률 5개만 추출
