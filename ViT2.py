...
def create_vit_classifier():
        ...
        exp = layers.experimental.preprocessing.Resizing(img_expanded_siz[0],img_expanded_siz[1])(nor)


        x=layers.experimental.preprocessing.RandomFlip('horizontal')(exp)
        x = layers.experimental.preprocessing.RandomRotation(factor=0.02)(x)
        x = layers.experimental.preprocessing.RandomZoom(height_factor=0.2,width_factor=0.2)(x)

        pathces = Patches(patch_siz)(x)
        x = PatchEncoder(p2,d_model)(pathces)
